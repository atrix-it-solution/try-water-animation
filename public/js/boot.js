!(function () {
  function getCspNonce() {
    if (document.currentScript && document.currentScript.nonce)
      return document.currentScript.nonce;
    var e = document.querySelector("script[nonce]");
    if (e && e.nonce) return e.nonce;
    var t = document.querySelector('meta[name="csp-nonce"]');
    return t ? t.content : null;
  }
  var nonce = getCspNonce();
  if (nonce) {
    var originalCreateElement = document.createElement.bind(document);
    document.createElement = function (tagName, options) {
      var element = originalCreateElement(tagName, options);
      if (tagName.toLowerCase() === "style") {
        element.nonce = nonce;
        element.setAttribute("nonce", nonce);
      }
      return element;
    };
  }
  function appendScript(e, t) {
    var n = document.createElement("script");
    ((n.type = "text/javascript"), (n.defer = n.async = !0));
    var r = getCspNonce();
    r && ((n.nonce = r), n.setAttribute("nonce", r));
    if ("string" == typeof e) n.src = e;
    else if (e.textContent) n.textContent = e.textContent;
    t && (t.async && (n.async = t.async), t.defer && (n.defer = t.defer));
    return (document.getElementsByTagName("head")[0].appendChild(n), n);
  }
  var e = {
    organization: { code: "zig_ai" },
    environments: [
      { code: "production", pattern: "Lio=", hash: "6151995855962519263" },
    ],
    identities: {
      email: {
        type: "queryString",
        variable: "email",
        format: "string",
        encoding: "none",
        typeCode: "email",
      },
      swb_website_smart_tag: { type: "managedCookie", variable: "_swb" },
    },
    scripts: ["https://cdn.ketchjs.com/ketchtag/stable/v2.12/ketch-sdk.js"],
    languages: [{ code: "en", englishName: "English", nativeName: "English" }],
    services: {
      gvlHost: "https://global.ketchcdn.com/gvl/eu",
      portholeHost: "https://zig_ai.privacyportal.co",
      scriptHost: "https://cdn.ketchjs.com",
      shoreline: "https://global.ketchcdn.com/web/v3",
      telemetry: "https://global.ketchcdn.com/web/v2/log",
    },
    options: {
      appDivs: "hubspot-messages-iframe-container",
      beaconPercentage: "1",
    },
    property: {
      code: "website_smart_tag",
      name: "Website Smart Tag",
      platform: "WEB",
    },
    jurisdiction: {
      defaultScopeCode: "default",
      scopes: {
        AT: "gdpr",
        BE: "gdpr",
        BG: "gdpr",
        "CA-AB": "Canada",
        "CA-BC": "Canada",
        "CA-MB": "Canada",
        "CA-NB": "Canada",
        "CA-NL": "Canada",
        "CA-NS": "Canada",
        "CA-NT": "Canada",
        "CA-NU": "Canada",
        "CA-ON": "Canada",
        "CA-PE": "Canada",
        "CA-QC": "Quebec",
        "CA-SK": "Canada",
        "CA-YT": "Canada",
        CH: "gdpr",
        CY: "gdpr",
        CZ: "gdpr",
        DE: "gdpr",
        DK: "gdpr",
        EE: "gdpr",
        ES: "gdpr",
        FI: "gdpr",
        FO: "gdpr",
        FR: "gdpr",
        GB: "gdpr",
        GF: "gdpr",
        GG: "gdpr",
        GP: "gdpr",
        GR: "gdpr",
        HR: "gdpr",
        HU: "gdpr",
        IE: "gdpr",
        IM: "gdpr",
        IS: "gdpr",
        IT: "gdpr",
        JE: "gdpr",
        LI: "gdpr",
        LT: "gdpr",
        LU: "gdpr",
        LV: "gdpr",
        MF: "gdpr",
        MQ: "gdpr",
        MT: "gdpr",
        NL: "gdpr",
        NO: "gdpr",
        PL: "gdpr",
        PT: "gdpr",
        RE: "gdpr",
        RO: "gdpr",
        SE: "gdpr",
        SI: "gdpr",
        SJ: "gdpr",
        SK: "gdpr",
        "US-CA": "US_California",
        "US-CO": "US_Colorado",
        "US-CT": "US_Connecticut",
        "US-DE": "US_Delaware",
        "US-IA": "US_Iowa",
        "US-MN": "US_Minnesota",
        "US-MT": "US_Montana",
        "US-NE": "US_Nebraska",
        "US-NH": "US_NewHampshire",
        "US-NJ": "US_NewJersey",
        "US-OR": "US_Oregon",
        "US-TN": "US_Tennessee",
        "US-TX": "US_Texas",
        "US-UT": "US_Utah",
        "US-VA": "US_Virginia",
        YT: "gdpr",
      },
      defaultJurisdictionCode: "default",
      defaultJurisdictionName: "Global Standard",
      jurisdictions: {
        AT: "gdpr",
        BE: "gdpr",
        BG: "gdpr",
        "CA-AB": "Canada",
        "CA-BC": "Canada",
        "CA-MB": "Canada",
        "CA-NB": "Canada",
        "CA-NL": "Canada",
        "CA-NS": "Canada",
        "CA-NT": "Canada",
        "CA-NU": "Canada",
        "CA-ON": "Canada",
        "CA-PE": "Canada",
        "CA-QC": "Quebec",
        "CA-SK": "Canada",
        "CA-YT": "Canada",
        CH: "gdpr",
        CY: "gdpr",
        CZ: "gdpr",
        DE: "gdpr",
        DK: "gdpr",
        EE: "gdpr",
        ES: "gdpr",
        FI: "gdpr",
        FO: "gdpr",
        FR: "gdpr",
        GB: "gdpr",
        GF: "gdpr",
        GG: "gdpr",
        GP: "gdpr",
        GR: "gdpr",
        HR: "gdpr",
        HU: "gdpr",
        IE: "gdpr",
        IM: "gdpr",
        IS: "gdpr",
        IT: "gdpr",
        JE: "gdpr",
        LI: "gdpr",
        LT: "gdpr",
        LU: "gdpr",
        LV: "gdpr",
        MF: "gdpr",
        MQ: "gdpr",
        MT: "gdpr",
        NL: "gdpr",
        NO: "gdpr",
        PL: "gdpr",
        PT: "gdpr",
        RE: "gdpr",
        RO: "gdpr",
        SE: "gdpr",
        SI: "gdpr",
        SJ: "gdpr",
        SK: "gdpr",
        "US-CA": "US_California",
        "US-CO": "US_Colorado",
        "US-CT": "US_Connecticut",
        "US-DE": "US_Delaware",
        "US-IA": "US_Iowa",
        "US-MN": "US_Minnesota",
        "US-MT": "US_Montana",
        "US-NE": "US_Nebraska",
        "US-NH": "US_NewHampshire",
        "US-NJ": "US_NewJersey",
        "US-OR": "US_Oregon",
        "US-TN": "US_Tennessee",
        "US-TX": "US_Texas",
        "US-UT": "US_Utah",
        "US-VA": "US_Virginia",
        YT: "gdpr",
      },
      regions: {
        AD: { name: "Andorra" },
        AE: { name: "United Arab Emirates" },
        AF: { name: "Afghanistan" },
        AG: { name: "Antigua and Barbuda" },
        AI: { name: "Anguilla" },
        AL: { name: "Albania" },
        AM: { name: "Armenia" },
        AO: { name: "Angola" },
        APAC: { name: "Asia-Pacific" },
        AQ: { name: "Antarctica" },
        AR: { name: "Argentina" },
        AS: { name: "American Samoa" },
        AT: {
          name: "Austria",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        AU: { name: "Australia" },
        AW: { name: "Aruba" },
        AX: { name: "Åland Islands" },
        AZ: { name: "Azerbaijan" },
        BA: { name: "Bosnia and Herzegovina" },
        BB: { name: "Barbados" },
        BD: { name: "Bangladesh" },
        BE: {
          name: "Belgium",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        BF: { name: "Burkina Faso" },
        BG: {
          name: "Bulgaria",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        BH: { name: "Bahrain" },
        BI: { name: "Burundi" },
        BJ: { name: "Benin" },
        BL: { name: "Saint Barthélemy" },
        BM: { name: "Bermuda" },
        BN: { name: "Brunei" },
        BO: { name: "Bolivia" },
        BQ: { name: "Bonaire" },
        BR: { name: "Brazil" },
        BS: { name: "Bahamas" },
        BT: { name: "Bhutan" },
        BV: { name: "Bouvet Island" },
        BW: { name: "Botswana" },
        BY: { name: "Belarus" },
        BZ: { name: "Belize" },
        CA: { name: "Canada" },
        "CA-AB": {
          name: "Alberta",
          jurisdictionCode: "Canada",
          jurisdictionName: "Canada",
        },
        "CA-BC": {
          name: "British Columbia",
          jurisdictionCode: "Canada",
          jurisdictionName: "Canada",
        },
        "CA-MB": {
          name: "Manitoba",
          jurisdictionCode: "Canada",
          jurisdictionName: "Canada",
        },
        "CA-NB": {
          name: "New Brunswick",
          jurisdictionCode: "Canada",
          jurisdictionName: "Canada",
        },
        "CA-NL": {
          name: "Newfoundland and Labrador",
          jurisdictionCode: "Canada",
          jurisdictionName: "Canada",
        },
        "CA-NS": {
          name: "Nova Scotia",
          jurisdictionCode: "Canada",
          jurisdictionName: "Canada",
        },
        "CA-NT": {
          name: "Northwest Territories",
          jurisdictionCode: "Canada",
          jurisdictionName: "Canada",
        },
        "CA-NU": {
          name: "Nunavut",
          jurisdictionCode: "Canada",
          jurisdictionName: "Canada",
        },
        "CA-ON": {
          name: "Ontario",
          jurisdictionCode: "Canada",
          jurisdictionName: "Canada",
        },
        "CA-PE": {
          name: "Prince Edward Island",
          jurisdictionCode: "Canada",
          jurisdictionName: "Canada",
        },
        "CA-QC": {
          name: "Quebec",
          jurisdictionCode: "Quebec",
          jurisdictionName: "Quebec",
        },
        "CA-SK": {
          name: "Saskatchewan",
          jurisdictionCode: "Canada",
          jurisdictionName: "Canada",
        },
        "CA-YT": {
          name: "Yukon",
          jurisdictionCode: "Canada",
          jurisdictionName: "Canada",
        },
        CC: { name: "Cocos (Keeling) Islands" },
        CD: { name: "Democratic Republic of the Congo" },
        CF: { name: "Central African Republic" },
        CG: { name: "Republic of the Congo" },
        CH: {
          name: "Switzerland",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        CI: { name: "Côte d’Ivoire" },
        CK: { name: "Cook Islands" },
        CL: { name: "Chile" },
        CM: { name: "Cameroon" },
        CN: { name: "China" },
        CO: { name: "Colombia" },
        CR: { name: "Costa Rica" },
        CU: { name: "Cuba" },
        CV: { name: "Cabo Verde" },
        CW: { name: "Curaçao" },
        CX: { name: "Christmas Island" },
        CY: {
          name: "Cyprus",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        CZ: {
          name: "Czech Republic",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        DE: {
          name: "Germany",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        DJ: { name: "Djibouti" },
        DK: {
          name: "Denmark",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        DM: { name: "Dominica" },
        DO: { name: "Dominican Republic" },
        DZ: { name: "Algeria" },
        EC: { name: "Ecuador" },
        EE: {
          name: "Estonia",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        EEA: { name: "European Economic Area" },
        EG: { name: "Egypt" },
        EH: { name: "Western Sahara" },
        ER: { name: "Eritrea" },
        ES: {
          name: "Spain",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        ET: { name: "Ethiopia" },
        EU: { name: "European Union" },
        FI: {
          name: "Finland",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        FJ: { name: "Fiji" },
        FK: { name: "Falkland Islands" },
        FM: { name: "Micronesia" },
        FO: {
          name: "Faroe Islands",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        FR: {
          name: "France",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        GA: { name: "Gabon" },
        GB: {
          name: "United Kingdom",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        GD: { name: "Grenada" },
        GE: { name: "Georgia" },
        GF: {
          name: "French Guiana",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        GG: {
          name: "Guernsey",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        GH: { name: "Ghana" },
        GI: { name: "Gibraltar" },
        GL: { name: "Greenland" },
        GM: { name: "Gambia" },
        GN: { name: "Guinea" },
        GP: {
          name: "Guadeloupe",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        GQ: { name: "Equatorial Guinea" },
        GR: {
          name: "Greece",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        GS: { name: "South Georgia and the South Sandwich Islands" },
        GT: { name: "Guatemala" },
        GU: { name: "Guam" },
        GW: { name: "Guinea-Bissau" },
        GY: { name: "Guyana" },
        HK: { name: "Hong Kong" },
        HM: { name: "Heard Island and McDonald Islands" },
        HN: { name: "Honduras" },
        HR: {
          name: "Croatia",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        HT: { name: "Haiti" },
        HU: {
          name: "Hungary",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        ID: { name: "Indonesia" },
        IE: {
          name: "Ireland",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        IL: { name: "Israel" },
        IM: {
          name: "Isle of Man",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        IN: { name: "India" },
        IO: { name: "British Indian Ocean Territory" },
        IQ: { name: "Iraq" },
        IR: { name: "Iran" },
        IS: {
          name: "Iceland",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        IT: {
          name: "Italy",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        JE: {
          name: "Jersey",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        JM: { name: "Jamaica" },
        JO: { name: "Jordan" },
        JP: { name: "Japan" },
        KE: { name: "Kenya" },
        KG: { name: "Kyrgyzstan" },
        KH: { name: "Cambodia" },
        KI: { name: "Kiribati" },
        KM: { name: "Comoros" },
        KN: { name: "Saint Kitts and Nevis" },
        KP: { name: "North Korea" },
        KR: { name: "South Korea" },
        KW: { name: "Kuwait" },
        KY: { name: "Cayman Islands" },
        KZ: { name: "Kazakhstan" },
        LA: { name: "Laos" },
        LATAM: { name: "Latin America" },
        LB: { name: "Lebanon" },
        LC: { name: "Saint Lucia" },
        LI: {
          name: "Liechtenstein",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        LK: { name: "Sri Lanka" },
        LR: { name: "Liberia" },
        LS: { name: "Lesotho" },
        LT: {
          name: "Lithuania",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        LU: {
          name: "Luxembourg",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        LV: {
          name: "Latvia",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        LY: { name: "Libya" },
        MA: { name: "Morocco" },
        MC: { name: "Monaco" },
        MD: { name: "Moldova" },
        ME: { name: "Montenegro" },
        MF: {
          name: "Saint Martin",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        MG: { name: "Madagascar" },
        MH: { name: "Marshall Islands" },
        MK: { name: "North Macedonia" },
        ML: { name: "Mali" },
        MM: { name: "Myanmar" },
        MN: { name: "Mongolia" },
        MO: { name: "Macao" },
        MP: { name: "Northern Mariana Islands" },
        MQ: {
          name: "Martinique",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        MR: { name: "Mauritania" },
        MS: { name: "Montserrat" },
        MT: {
          name: "Malta",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        MU: { name: "Mauritius" },
        MV: { name: "Maldives" },
        MW: { name: "Malawi" },
        MX: { name: "Mexico" },
        MY: { name: "Malaysia" },
        MZ: { name: "Mozambique" },
        NA: { name: "Namibia" },
        NC: { name: "New Caledonia" },
        NE: { name: "Niger" },
        NF: { name: "Norfolk Island" },
        NG: { name: "Nigeria" },
        NI: { name: "Nicaragua" },
        NL: {
          name: "Netherlands",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        NO: {
          name: "Norway",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        NP: { name: "Nepal" },
        NZ: { name: "New Zealand" },
        OM: { name: "Oman" },
        PA: { name: "Panama" },
        PE: { name: "Peru" },
        PF: { name: "French Polynesia" },
        PG: { name: "Papua New Guinea" },
        PH: { name: "Philippines" },
        PK: { name: "Pakistan" },
        PL: {
          name: "Poland",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        PM: { name: "Saint Pierre and Miquelon" },
        PN: { name: "Pitcairn" },
        PR: { name: "Puerto Rico" },
        PS: { name: "Palestine" },
        PT: {
          name: "Portugal",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        PW: { name: "Palau" },
        PY: { name: "Paraguay" },
        QA: { name: "Qatar" },
        RE: {
          name: "Réunion",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        RO: {
          name: "Romania",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        RS: { name: "Serbia" },
        RU: { name: "Russia" },
        RW: { name: "Rwanda" },
        SA: { name: "Saudi Arabia" },
        SB: { name: "Solomon Islands" },
        SC: { name: "Seychelles" },
        SD: { name: "Sudan" },
        SE: {
          name: "Sweden",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        SG: { name: "Singapore" },
        SH: { name: "Saint Helena" },
        SI: {
          name: "Slovenia",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        SJ: {
          name: "Svalbard and Jan Mayen Islands",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        SK: {
          name: "Slovakia",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        SL: { name: "Sierra Leone" },
        SM: { name: "San Marino" },
        SN: { name: "Senegal" },
        SO: { name: "Somalia" },
        SR: { name: "Suriname" },
        SS: { name: "South Sudan" },
        ST: { name: "Sao Tome and Principe" },
        SV: { name: "El Salvador" },
        SX: { name: "Sint Maarten" },
        SY: { name: "Syria" },
        SZ: { name: "Eswatini" },
        TC: { name: "Turks and Caicos Islands" },
        TD: { name: "Chad" },
        TF: { name: "French Southern and Antarctic Lands" },
        TG: { name: "Togo" },
        TH: { name: "Thailand" },
        TJ: { name: "Tajikistan" },
        TK: { name: "Tokelau" },
        TL: { name: "Timor-Leste" },
        TM: { name: "Turkmenistan" },
        TN: { name: "Tunisia" },
        TO: { name: "Tonga" },
        TR: { name: "Turkey" },
        TT: { name: "Trinidad and Tobago" },
        TV: { name: "Tuvalu" },
        TW: { name: "Taiwan" },
        TZ: { name: "Tanzania" },
        UA: { name: "Ukraine" },
        UG: { name: "Uganda" },
        UM: { name: "United States Minor Outlying Islands" },
        US: { name: "United States" },
        "US-AK": { name: "Alaska" },
        "US-AL": { name: "Alabama" },
        "US-AR": { name: "Arkansas" },
        "US-AZ": { name: "Arizona" },
        "US-CA": {
          name: "California",
          jurisdictionCode: "US_California",
          jurisdictionName: "US - California",
        },
        "US-CO": {
          name: "Colorado",
          jurisdictionCode: "US_Colorado",
          jurisdictionName: "US - Colorado",
        },
        "US-CT": {
          name: "Connecticut",
          jurisdictionCode: "US_Connecticut",
          jurisdictionName: "US - Connecticut",
        },
        "US-DC": { name: "Washington, D.C." },
        "US-DE": {
          name: "Delaware",
          jurisdictionCode: "US_Delaware",
          jurisdictionName: "US - Delaware",
        },
        "US-FL": { name: "Florida" },
        "US-GA": { name: "Georgia" },
        "US-HI": { name: "Hawaii" },
        "US-IA": {
          name: "Iowa",
          jurisdictionCode: "US_Iowa",
          jurisdictionName: "US - Iowa",
        },
        "US-ID": { name: "Idaho" },
        "US-IL": { name: "Illinois" },
        "US-IN": { name: "Indiana" },
        "US-KS": { name: "Kansas" },
        "US-KY": { name: "Kentucky" },
        "US-LA": { name: "Louisiana" },
        "US-MA": { name: "Massachusetts" },
        "US-MD": { name: "Maryland" },
        "US-ME": { name: "Maine" },
        "US-MI": { name: "Michigan" },
        "US-MN": {
          name: "Minnesota",
          jurisdictionCode: "US_Minnesota",
          jurisdictionName: "US - Minnesota",
        },
        "US-MO": { name: "Missouri" },
        "US-MS": { name: "Mississippi" },
        "US-MT": {
          name: "Montana",
          jurisdictionCode: "US_Montana",
          jurisdictionName: "US - Montana",
        },
        "US-NC": { name: "North Carolina" },
        "US-ND": { name: "North Dakota" },
        "US-NE": {
          name: "Nebraska",
          jurisdictionCode: "US_Nebraska",
          jurisdictionName: "US - Nebraska",
        },
        "US-NH": {
          name: "New Hampshire",
          jurisdictionCode: "US_NewHampshire",
          jurisdictionName: "US - New Hampshire",
        },
        "US-NJ": {
          name: "New Jersey",
          jurisdictionCode: "US_NewJersey",
          jurisdictionName: "US - New Jersey",
        },
        "US-NM": { name: "New Mexico" },
        "US-NV": { name: "Nevada" },
        "US-NY": { name: "New York" },
        "US-OH": { name: "Ohio" },
        "US-OK": { name: "Oklahoma" },
        "US-OR": {
          name: "Oregon",
          jurisdictionCode: "US_Oregon",
          jurisdictionName: "US - Oregon",
        },
        "US-PA": { name: "Pennsylvania" },
        "US-RI": { name: "Rhode Island" },
        "US-SC": { name: "South Carolina" },
        "US-SD": { name: "South Dakota" },
        "US-TN": {
          name: "Tennessee",
          jurisdictionCode: "US_Tennessee",
          jurisdictionName: "US - Tennessee",
        },
        "US-TX": {
          name: "Texas",
          jurisdictionCode: "US_Texas",
          jurisdictionName: "US - Texas",
        },
        "US-UT": {
          name: "Utah",
          jurisdictionCode: "US_Utah",
          jurisdictionName: "US - Utah",
        },
        "US-VA": {
          name: "Virginia",
          jurisdictionCode: "US_Virginia",
          jurisdictionName: "US - Virginia",
        },
        "US-VT": { name: "Vermont" },
        "US-WA": { name: "Washington" },
        "US-WI": { name: "Wisconsin" },
        "US-WV": { name: "West Virginia" },
        "US-WY": { name: "Wyoming" },
        UY: { name: "Uruguay" },
        UZ: { name: "Uzbekistan" },
        VA: { name: "Holy See" },
        VC: { name: "Saint Vincent and the Grenadines" },
        VE: { name: "Venezuela" },
        VG: { name: "British Virgin Islands" },
        VI: { name: "US Virgin Islands" },
        VN: { name: "Vietnam" },
        VU: { name: "Vanuatu" },
        WF: { name: "Wallis and Futuna Islands" },
        WS: { name: "Samoa" },
        YE: { name: "Yemen" },
        YT: {
          name: "Mayotte",
          jurisdictionCode: "gdpr",
          jurisdictionName: "European Economic Area",
        },
        ZA: { name: "South Africa" },
        ZM: { name: "Zambia" },
        ZW: { name: "Zimbabwe" },
      },
    },
  };
  ((window.semaphore = window.semaphore || []),
    (window.ketch = function () {
      window.semaphore.push(arguments);
    }),
    window.semaphore.unshift(["init", e]));
  for (
    var n = document.getElementsByTagName("head")[0], t = 0;
    t < e.scripts.length;
    t++
  ) {
    appendScript(e.scripts[t]);
  }
})();
